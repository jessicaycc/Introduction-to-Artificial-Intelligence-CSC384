...
def prop_FC(csp, newVar=None):
    ...
    for c in con:
        if c.get_n_unasgn() == 1: 
            varX = c.get_unasgn_vars()[0]
            for d in varX.cur_domain():
                if not c.has_support(varX, d):
                    if (varX,d) not in pruned:
                        pruned.append((varX,d))
                        varX.prune_value(d)
            if varX.cur_domain_size() == 0:
                return False, pruned
    return True, pruned
                ...
                if not c.has_support(v, d):
                    ...
                    if v.cur_domain_size() == 0:
                        ...
                        for constraints in csp.get_cons_with_var(v):
                            if constraints not in GAC_Q and constraints is not c:
                                GAC_Q.append(constraints)
    return True, pruned           
